...
describe("ERC20_MONEZO_TOKEN", function () {
  let acc1;
  let acc2;
  let acc3;
  let payments;
  ...
  it("Fuction name(), symbol(), decimals(), totalSupply(), cap() - can show all this", async function () {
    ...
    expect(await NotOwner.name()).to.equal("MONEZOTOKEN");
    expect(await NotOwner.symbol()).to.equal("MZ");
    expect(await NotOwner.decimals()).to.equal(18);
    expect((await NotOwner.totalSupply()) / 1000000000000000000).to.equal(100);
    expect((await NotOwner.cap()) / 1000000000000000000).to.equal(1000000000);
  });
  ...
  it("Function _mint() - user can mint tokens", async function () {
    ...
    await expect(
      payments._mint(acc1.address, 1000000000000000000000000000n)
  ...
  it("Function _burn() - usr can burn tokens", async function () {
    ...
    expect(await payments.cap()).to.equal(
      1000000000000000000000000000n - 5000000n
    );
  });
  ...
  it("Function withdraw() - Owner can withdraw ETH", async function () {
    tx = {
      to: payments.address,
      value: 10000,
    };
